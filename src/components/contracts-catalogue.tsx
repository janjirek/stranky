...
export default function ContractsCatalogue({
  ...
  return (
    <div className="mb-8 py-12 md:py-20">
      {contracts.slice(0, visiblePosts).map((post, postIndex) => {
        const detailURL = `/${post.data.slug}`;

        return (
          // 1. CELÁ KARTA JE KLIKACÍ - ODSTRANĚNO ZAOBLENÍ (rounded-2xl)
          <a key={post.id} href={detailURL} className="group block relative">
            <article className="bg-white dark:bg-slate-800 p-6 shadow-lg hover:shadow-xl transition-all duration-300 mb-8 border border-slate-100 dark:border-slate-700">
                ...
                <div className="flex flex-col flex-grow pt-2">
                  ...
                  <footer className="mt-auto pt-4 border-t border-slate-100 dark:border-slate-700">
                    <div className="text-sm font-medium text-blue-600 transition-colors group-hover:text-blue-700 flex items-center gap-1">
                      ...
                      <span className="tracking-normal text-blue-300 transition-transform duration-150 ease-in-out group-hover:translate-x-0.5">
                        →
                      </span>
                    </div>
                  </footer>
